...
export default function LibrarianDashboard() {
  ...
  useEffect(() => { fetchBooks(); fetchRequests(); }, []);

  const fetchBooks = async () => { const res = await api.get('/books'); setBooks(res.data); };
  const fetchRequests = async () => { const res = await api.get('/borrow/requests'); setRequests(res.data); };

  const handleFormSubmit = async (e) => {
    e.preventDefault();
    try {
      if (isEditing) {
        await api.put(`/books/${editId}`, { ...formData, totalCopies: formData.quantity });
  ...
  const startEdit = (book) => {
    ...
    setFormData({
      bookISBN: book.bookISBN, bookTitle: book.bookTitle, bookAuthor: book.bookAuthor || '',
      bookGenre: book.bookGenre, quantity: book.totalCopies 
    });
  ...
  const handleDelete = async (id, available, total) => {
    if (available < total) {
      alert(`⚠️ Cannot delete!\n\n${total - available} copy(ies) are out.`); return;
    }
    if (!window.confirm("Are you sure? This will PERMANENTLY delete the book.")) return;
    try { await api.delete(`/books/${id}`); fetchBooks(); } catch (err) { alert(err.response?.data?.error); }
  };

  const handleApprove = async (borrowID) => {
    const daysStr = prompt("Enter number of days:", "7");
    if (!daysStr) return;
    try { await api.post('/borrow/approve', { borrowID, days: parseInt(daysStr) }); fetchRequests(); fetchBooks(); } 
    catch (err) { alert('Error approving'); }
  };

  const handleConfirmReturn = async (borrowID) => {
    if(!window.confirm("Confirm return?")) return;
    try { const res = await api.post('/return', { borrowID }); alert(res.data.message); fetchRequests(); fetchBooks(); } 
    catch (err) { alert('Error returning'); }
  };

  const handleManualReturn = async () => {
    if(!returnId) return alert("Enter ID");
    try { const res = await api.post('/return', { borrowID: returnId }); alert(res.data.message); setReturnId(''); fetchBooks(); } 
    catch (err) { alert('Error returning'); }
  };
  ...
  const filteredBooks = books.filter(book => {
    ...
    return (
      ...
      book.bookISBN.startsWith(lowerSearch)
    );
  });
  ...
  return (
    <div className="container">
      ...
      <div className="section warning">
        ...
        {requests.length === 0 ? <p>No pending tasks.</p> : (
          <table className="task-table">
            <thead>
              <tr><th>User</th><th>Book</th><th>Action</th></tr>
            </thead>
            <tbody>
              {requests.map(req => (
                <tr key={req.borrowID}>
                  <td>{req.borrowerName}</td><td>{req.bookTitle}</td>
                  <td>
                    {req.status === 'pending' ? (
      ...
      <div ref={formRef} className={`section ${isEditing ? 'primary' : ''}`}>
        <h3 className="text-center" style={{color: isEditing ? '#007bff' : 'black'}}>
        ...
        <form onSubmit={handleFormSubmit} className="form-group">
          ...
          <div className="form-row">
            ...
            {isEditing && <button type="button" onClick={cancelEdit} className="btn-secondary">Cancel</button>}
          </div>
        </form>
      </div>
      ...
      <ul className="book-list">
        {filteredBooks.length === 0 ? <p className="text-muted text-center">No books found matching your search.</p> : (
          filteredBooks.map(book => (
            <li key={book.bookID} className="book-list-item">
              ...
              <div>
                <button onClick={() => startEdit(book)} className="btn-warning btn-sm mr-2">Edit</button>
                <button onClick={() => handleDelete(book.bookID, book.availableCopies, book.totalCopies)} className="btn-danger btn-sm" style={{ opacity: (book.availableCopies === book.totalCopies) ? 1 : 0.6 }}>Delete</button>
              </div>
            </li>
          ))
        )}
